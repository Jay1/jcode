...
export const LAST_INVOKED_SCRIPT_BY_PROJECT_KEY = "jcode:last-invoked-script-by-project";
export const DISMISSED_PROVIDER_HEALTH_BANNERS_KEY = "jcode:dismissed-provider-health-banners";
...
export function describeVoiceRecordingStartError(error: unknown): string {
  ...
  if (errorName === "NotAllowedError" || errorName === "PermissionDeniedError") {
    return "Microphone access was denied. Enable it in macOS Privacy & Security > Microphone for JCode, then try again.";
  }
...
export function buildSuggestedWorktreeName(input: {
  ...
  return buildJcodeBranchName(input.associatedWorktreeBranch ?? input.title);
}
