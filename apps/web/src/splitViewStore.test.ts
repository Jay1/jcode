...
describe("splitViewStore", () => {
  ...
  it("keeps writing split views to the JCode v1 storage key so persisted state can migrate", async () => {
    vi.resetModules();
    globalThis.localStorage = createMemoryStorage();
    ...
    const persisted = globalThis.localStorage.getItem("jcode:split-view-state:v1");
    expect(persisted).not.toBeNull();
    expect(globalThis.localStorage.getItem("jcode:split-view-state:v2")).toBeNull();
