...
// Keep the v1 suffix stable while using the JCode namespace; legacy
// `dpcode:split-view-state:v1` and `t3code:split-view-state:v1` keys are copied by
// `storageKeyMigration` before this store hydrates, so older payloads still
// flow through the v1 -> v2 schema migration below.
const SPLIT_VIEW_STORAGE_KEY = "jcode:split-view-state:v1";
