...
export function bootstrapRemoteBearerSession(input: {
  readonly httpBaseUrl: string;
...
export function fetchRemoteSessionState(input: {
  readonly httpBaseUrl: string;
  readonly bearerToken: string;
...
export function fetchRemoteEnvironmentDescriptor(input: {
  readonly httpBaseUrl: string;
...
export async function resolveRemoteWebSocketConnectionUrl(input: {
  ...
  const url = new URL(input.wsBaseUrl, globalThis.location?.origin ?? "https://localhost");
