...
// Purpose: Resolves and seeds Electron userData profile paths during the JCode rename.
...
const DEV_USER_DATA_DIR_NAME = "jcode-dev";
const PROD_USER_DATA_DIR_NAME = "jcode";
...
export function seedDesktopUserDataProfileFromLegacy(input: {
  ...
  try {
    ...
    FS.writeFileSync(
      Path.join(input.targetPath, "jcode-profile-seed.json"),
      `${JSON.stringify(
        {
          sourcePath,
