...
function cleanupStaleDevApps() {
  ...
  spawnSync("pkill", ["-f", "--", `--jcode-dev-root=${desktopDir}`], { stdio: "ignore" });
}
...
function listStaleComputerUsePids() {
  ...
  const result = spawnSync("pgrep", ["-fal", "JCode \\(Dev\\).*(computerUseMcp\\.mjs mcp)"], {
    encoding: "utf8",
  });
...
function cleanupStaleComputerUseApps() {
  ...
  console.error(
    `[desktop-dev] Cleaning up ${stalePids.length} stale JCode (Dev) Computer Use helper process${stalePids.length === 1 ? "" : "es"} from other worktrees.`,
  );
...
function startApp() {
  ...
  const app = spawn(
    resolveElectronPath(),
    [`--jcode-dev-root=${desktopDir}`, "dist-electron/main.js"],
    {
      cwd: desktopDir,
      env: {
        ...childEnv,
