  ...
  describe("stashAndCheckout", () => {
    ...
    it.effect("drops only the temporary stash it created after a successful reapply", () =>
      Effect.gen(function* () {
        ...
        expect(stashList).not.toContain("jcode: stash before switching to feature");
        expect(yield* readTextFile(path.join(tmp, "README.md"))).toBe("dirty changes\n");
      }),
    );
    ...
    it.effect("keeps the stash when reapplying dirty changes conflicts", () =>
      Effect.gen(function* () {
        ...
        expect(yield* git(tmp, ["stash", "list"])).toContain(
          "jcode: stash before switching to conflicting",
        );
      }),
    );
  });
  ...
  describe("renameGitBranch", () => {
    ...
    it.effect("appends numeric suffix when target branch already exists", () =>
      Effect.gen(function* () {
        const tmp = yield* makeTmpDir();
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "jcode/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "jcode/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "jcode/tmp-working" });

        const renamed = yield* (yield* GitCore).renameBranch({
          cwd: tmp,
          oldBranch: "jcode/tmp-working",
          newBranch: "jcode/feat/session",
        });

        expect(renamed.branch).toBe("jcode/feat/session-1");
        const branches = yield* (yield* GitCore).listBranches({ cwd: tmp });
        expect(branches.branches.some((branch) => branch.name === "jcode/feat/session")).toBe(true);
        expect(branches.branches.some((branch) => branch.name === "jcode/feat/session-1")).toBe(
          true,
        );
        const current = branches.branches.find((branch) => branch.current);
        expect(current?.name).toBe("jcode/feat/session-1");
      }),
    );
    ...
    it.effect("increments suffix until it finds an available branch name", () =>
      Effect.gen(function* () {
        ...
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "jcode/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "jcode/feat/session-1" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "jcode/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "jcode/tmp-working" });
        ...
        expect(renamed.branch).toBe("jcode/feat/session-2");
      }),
    );
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          yield* git(tmp, ["remote", "add", "origin", "git@github.com:pingdotgg/jcode.git"]);

          const remoteName = yield* core.ensureRemote({
            cwd: tmp,
            preferredName: "origin",
            url: "git@github.com:pingdotgg/jcode.git/",
          });
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(
            yield* git(tmp, ["ls-remote", "--heads", "jasonLaster", "jcode/pr-488/statemachine"]),
          ).toBe("");
        }),
    );
