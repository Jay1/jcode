...
function buildCodexWrapperScript(input: {
  ...
  return [
    ...
    '    _jcode_codex_ts="$(date +%s 2>/dev/null || echo "$$")"',
    '    export CODEX_TUI_SESSION_LOG_PATH="${TMPDIR:-/tmp}/jcode-codex-session-$$_${_jcode_codex_ts}.jsonl"',
    "  fi",
    "  (",
    '    _jcode_log="$CODEX_TUI_SESSION_LOG_PATH"',
    `    _jcode_notify=${shellQuote(notifyHookPath)}`,
    '    _jcode_last_turn_id=""',
    '    _jcode_last_approval_id=""',
    '    _jcode_last_exec_call_id=""',
    "    _jcode_approval_fallback_seq=0",
    "",
    "    _jcode_emit_event() {",
    '      _jcode_event="$1"',
    `      _jcode_payload=$(printf '{"hook_event_name":"%s"}' "$_jcode_event")`,
    '      "$_jcode_notify" "$_jcode_payload" >/dev/null 2>&1 || true',
    "    }",
    "",
    "    _jcode_i=0",
    '    while [ ! -f "$_jcode_log" ] && [ "$_jcode_i" -lt 200 ]; do',
    "      _jcode_i=$((_jcode_i + 1))",
    "      sleep 0.05",
    "    done",
    '    if [ ! -f "$_jcode_log" ]; then',
    "      exit 0",
    "    fi",
    "",
    '    tail -n 0 -F "$_jcode_log" 2>/dev/null | while IFS= read -r _jcode_line; do',
    '      case "$_jcode_line" in',
    `        *'"dir":"to_tui"'*'"kind":"codex_event"'*'"msg":{"type":"task_started"'*)`,
    `          _jcode_turn_id=$(printf '%s\n' "$_jcode_line" | awk -F'"turn_id":"' 'NF > 1 { sub(/".*/, "", $2); print $2; exit }')`,
    '          [ -n "$_jcode_turn_id" ] || _jcode_turn_id="task_started"',
    '          if [ "$_jcode_turn_id" != "$_jcode_last_turn_id" ]; then',
    '            _jcode_last_turn_id="$_jcode_turn_id"',
    '            _jcode_emit_event "Start"',
    "          fi",
    "          ;;",
    `        *'"dir":"to_tui"'*'"kind":"codex_event"'*'"msg":{"type":"'*'_approval_request"'*)`,
    `          _jcode_approval_id=$(printf '%s\n' "$_jcode_line" | awk -F'"id":"' 'NF > 1 { sub(/".*/, "", $2); print $2; exit }')`,
    `          [ -n "$_jcode_approval_id" ] || _jcode_approval_id=$(printf '%s\n' "$_jcode_line" | awk -F'"approval_id":"' 'NF > 1 { sub(/".*/, "", $2); print $2; exit }')`,
    `          [ -n "$_jcode_approval_id" ] || _jcode_approval_id=$(printf '%s\n' "$_jcode_line" | awk -F'"call_id":"' 'NF > 1 { sub(/".*/, "", $2); print $2; exit }')`,
    '          if [ -z "$_jcode_approval_id" ]; then',
    "            _jcode_approval_fallback_seq=$((_jcode_approval_fallback_seq + 1))",
    '            _jcode_approval_id="approval_request_${_jcode_approval_fallback_seq}"',
    "          fi",
    '          if [ "$_jcode_approval_id" != "$_jcode_last_approval_id" ]; then',
    '            _jcode_last_approval_id="$_jcode_approval_id"',
    '            _jcode_emit_event "PermissionRequest"',
    "          fi",
    "          ;;",
    `        *'"dir":"to_tui"'*'"kind":"codex_event"'*'"msg":{"type":"exec_command_begin"'*)`,
    `          _jcode_exec_call_id=$(printf '%s\n' "$_jcode_line" | awk -F'"call_id":"' 'NF > 1 { sub(/".*/, "", $2); print $2; exit }')`,
    '          if [ -n "$_jcode_exec_call_id" ]; then',
    '            if [ "$_jcode_exec_call_id" != "$_jcode_last_exec_call_id" ]; then',
    '              _jcode_last_exec_call_id="$_jcode_exec_call_id"',
    '              _jcode_emit_event "Start"',
    "            fi",
    "          else",
    '            _jcode_emit_event "Start"',
