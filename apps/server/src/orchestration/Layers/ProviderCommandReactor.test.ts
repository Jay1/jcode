...
describe("ProviderCommandReactor", () => {
  ...
  async function createHarness(input?: {
    ...
    const baseDir = input?.baseDir ?? fs.mkdtempSync(path.join(os.tmpdir(), "jcode-reactor-"));
  ...
  it("renames temporary worktree branches and keeps associated worktree metadata in sync", async () => {
    ...
    await waitFor(async () => {
      ...
      return (
        thread?.branch === "jcode/app-startup-crash" &&
        thread.associatedWorktreeBranch === "jcode/app-startup-crash" &&
        thread.associatedWorktreeRef === "jcode/app-startup-crash"
      );
    });
    ...
    expect(thread).toMatchObject({
      branch: "jcode/app-startup-crash",
      worktreePath: "/tmp/provider-project/.worktrees/cb661f0d",
      associatedWorktreePath: "/tmp/provider-project/.worktrees/cb661f0d",
      associatedWorktreeBranch: "jcode/app-startup-crash",
      associatedWorktreeRef: "jcode/app-startup-crash",
    });
  });
