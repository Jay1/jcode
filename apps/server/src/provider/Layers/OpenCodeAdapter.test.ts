...
function createMockOpenCodeRuntime(options?: {
  ...
  return { abortCalls, createCalls, promptCalls, runtime, sessionGetCalls, sessionUpdateCalls };
}
...
describe("OpenCodeAdapter runtime lifecycle", () => {
  ...
  it("sends plan-mode prompt instructions and captures tagged markdown as a proposed plan", async () => {
    ...
    expect(runtime.promptCalls[0]?.parts).toEqual([
      {
        type: "text",
        text: expect.stringContaining("JCode plan mode is active."),
      },
    ]);
    expect(result.map((event) => event.type)).toEqual([
      "session.started",
  ...
  it("does not treat idle after tool calls as an error when assistant text was emitted", async () => {
    const eventQueue = createSubscribedEventQueue();
    const runtime = createMockOpenCodeRuntime();
    const client = runtime.runtime.createOpenCodeSdkClient({
      baseUrl: "http://127.0.0.1:4099",
    ...
    const result = await Effect.runPromise(
      Effect.gen(function* () {
        ...
        yield* adapter.startSession({
          provider: "opencode",
          threadId: asThreadId("thread-idle-after-tool-text"),
          runtimeMode: "full-access",
        });
        ...
        eventQueue.push({
          ...
          properties: {
            sessionID: "opencode-session-1",
            part: {
              id: "part-tool-final-text",
              messageID: "msg-tool-final-text",
              type: "text",
              text: "done",
              time: {
                start: 1,
    ...
    expect(result).not.toContainEqual(
      expect.objectContaining({
        ...
        payload: expect.objectContaining({
          message: expect.stringContaining("became idle after tool calls"),
        }),
      }),
    );
  });
