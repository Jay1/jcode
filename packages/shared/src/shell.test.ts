...
describe("extractPathFromShellOutput", () => {
  ...
  it("still accepts legacy T3Code capture markers", () => {
    expect(
...
describe("readPathFromLoginShell", () => {
  it("uses a shell-agnostic printenv PATH probe", () => {
    ...
    >(() => "__JCODE_ENV_PATH_START__\n/a:/b\n__JCODE_ENV_PATH_END__\n");
    ...
    expect(args?.[1]).toContain("__JCODE_ENV_PATH_START__");
    expect(args?.[1]).toContain("__JCODE_ENV_PATH_END__");
...
describe("readEnvironmentFromLoginShell", () => {
  it("extracts multiple environment variables from a login shell command", () => {
    ...
    >(() =>
      [
        ...
        "/a:/b",
        "__JCODE_ENV_PATH_END__",
        "__JCODE_ENV_SSH_AUTH_SOCK_START__",
        "/tmp/secretive.sock",
        "__JCODE_ENV_SSH_AUTH_SOCK_END__",
      ].join("\n"),
    );
  ...
  it("preserves surrounding whitespace in captured values", () => {
    ...
    >(() =>
      ["__JCODE_ENV_CUSTOM_VAR_START__", "  padded value  ", "__JCODE_ENV_CUSTOM_VAR_END__"].join(
        "\n",
      ),
    );
  ...
  it("still parses legacy T3Code environment markers", () => {
    const execFile = vi.fn<
      (
        file: string,
        args: ReadonlyArray<string>,
