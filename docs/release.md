...
- Repository slug source:
  - `JCODE_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
  - otherwise legacy `T3CODE_DESKTOP_UPDATE_REPOSITORY`, if set.
  - otherwise `GITHUB_REPOSITORY` from GitHub Actions.
- Temporary private-repo auth workaround:
  - set `JCODE_DESKTOP_UPDATE_GITHUB_TOKEN` (or `GH_TOKEN`) in the desktop app runtime environment.
...
1. Confirm npm org/user owns package `@jcode/server` (or rename package first if needed).
