...
**Goal:** Add a Settings-native Skill Library that shows installed provider skills with dense searchable inventory, source filters, and read-only v1 management seams.
...
- `apps/web/src/lib/skillLibrary.ts`: pure helpers for provider-tagged skill rows, search filtering, and counts.
...
- [ ] Write failing helper tests for aggregating provider skill rows, provider counts, and search matching.
