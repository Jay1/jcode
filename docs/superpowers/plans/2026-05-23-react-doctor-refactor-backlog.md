...
- [ ] Repeat the same approach for `src/components/DiffPanel.tsx`. Started by extracting `DiffPanelShell` loading states; `DiffPanel.tsx` itself still needs a dedicated pass.
