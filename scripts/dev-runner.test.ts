...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  describe("resolveOffset", () => {
    it.effect("uses explicit JCODE_PORT_OFFSET when provided", () =>
      Effect.sync(() => {
        const result = resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "JCODE_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("throws for negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.ok(error.includes("Invalid JCODE_PORT_OFFSET"));
      }),
    );
  });

  describe("createDevRunnerEnv", () => {
    it.effect("defaults JCODE_HOME to ~/.jcode when not provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.JCODE_HOME, resolve(homedir(), ".jcode"));
        assert.equal(env.DPCODE_HOME, resolve(homedir(), ".jcode"));
        assert.equal(env.T3CODE_HOME, resolve(homedir(), ".jcode"));
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.JCODE_HOME, resolve("/tmp/custom-t3"));
        assert.equal(env.T3CODE_HOME, resolve("/tmp/custom-t3"));
        assert.equal(env.T3CODE_PORT, "4222");
        assert.equal(env.JCODE_PORT, "4222");
        assert.equal(env.VITE_WS_URL, "ws://[::1]:4222");
        assert.equal(env.JCODE_NO_BROWSER, "1");
        assert.equal(env.T3CODE_NO_BROWSER, "1");
        assert.equal(env.JCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.T3CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.JCODE_LOG_WS_EVENTS, "1");
        assert.equal(env.T3CODE_LOG_WS_EVENTS, "1");
        assert.equal(env.JCODE_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.JCODE_MODE, "web");
        assert.equal(env.T3CODE_LOG_WS_EVENTS, undefined);
        assert.equal(env.JCODE_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.JCODE_LOG_WS_EVENTS, "0");
    ...
    it.effect("uses custom t3Home when provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.JCODE_HOME, resolve("/tmp/my-t3"));
